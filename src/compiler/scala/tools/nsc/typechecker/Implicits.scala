...
//    assert(tree.isEmpty || tree.pos.isDefined, tree)
    ...
    class ImplicitComputation(iss: Infoss, shadowed: util.HashSet[Name]) {
      ...
      def survives(info: ImplicitInfo) = (
        ...
        && isPlausiblyCompatible(info.tpe, wildPt)  // optimization to avoid matchesPt   &&{ incCounter(plausiblyCompatibleImplicits) ; true}&&      // <--- cheaper than matchesPt
        && matchesPt(info)                          // stable and matches expected type  &&{ incCounter(matchingImplicits)            ; true}&&
      )
